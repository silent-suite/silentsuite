...
3. **Stage B — truthful public-served attestation (v3).** Dispatch `Annual-only Public Cutover (Stage B)` from `main` with the same exact SHA and Stage A identity. Its `annual-public-cutover` protected environment requires approval in addition to the normal `web-production` and `docs-production` environments. After every prior identity check, the attestation job freshly performs exact, no-cache `GET` probes of both `https://app.silentsuite.io/api/deployment-identity` and `https://docs.silentsuite.io/deployment-identity.json`; both must return HTTP 200 and the exact sole JSON member `{ "publicSha": "<expected_sha>" }`. It then signs the v3 body before any artifact exists: `publicArtifact` contains only the public repository, run, and attempt, with no self-referential artifact ID. The pinned official `actions/upload-artifact` action uploads that exact signed file and returns the finalized artifact ID and archive digest. A read-only API check binds those outputs to the public run, attempt, head, name, `expired=false`, and digest. Stage C receives the independent provenance (including the exact artifact ID and archive checksum) and dispatch identities; it must never infer the artifact ID from the signed body. The v3 signature, Stage A bytes digest, producer run, deployed runtime, predecessor bindings, and UTC-second timestamp ordering remain required.
