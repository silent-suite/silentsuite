...
test('only protected exact-SHA annual cutover can mint Stage B after freshly probing both live served identities', () => {
  const text = source('.github/workflows/annual-only-public-cutover.yml'); const probes = source('scripts/verify-annual-public-served-identities.mjs'); assert.match(text, /workflow_dispatch:/); assert.match(text, /github\.ref == 'refs\/heads\/main'/); assert.match(text, /github\.sha == inputs\.expected_sha/); assert.match(text, /environment: annual-public-cutover/); assert.match(text, /needs: \[deploy-web, deploy-docs\]/); assert.match(text, /ANNUAL_PUBLIC_SERVED_HMAC_KEY/); assert.match(text, /permission-actions: read/); assert.match(text, /Freshly verify exact production Web and Docs served identities before Stage B/); assert.match(text, /verify-annual-public-served-identities\.mjs/); assert.match(probes, /https:\/\/app\.silentsuite\.io\/api\/deployment-identity/); assert.match(probes, /https:\/\/docs\.silentsuite\.io\/deployment-identity\.json/); assert.match(text, /ANNUAL_PUBLIC_SERVED_CLIENT_SERVED_AT/); assert.match(text, /ANNUAL_DEPLOYMENT_VERIFIED: 'true'/); assert.match(text, /annual-only-public-served-attestation-\$\{\{ github\.run_id \}\}-\$\{\{ github\.run_attempt \}\}/); assert.doesNotMatch(text, /annual-only-final-cutover-manifest\.json/)
  const probe = text.indexOf('Freshly verify exact production Web and Docs served identities before Stage B'); const sign = text.indexOf('Sign truthful served attestation before any artifact exists'); const upload = text.indexOf('Upload exactly the signed served attestation through the pinned official artifact action'); const finalize = text.indexOf('Verify the finalized Stage B artifact record binds this exact run and head'); assert.ok(probe >= 0 && probe < sign && sign < upload && upload < finalize)
...
test('Stage B signs before upload, uses the exact pinned official artifact action, and exposes finalized outputs', () => {
  const workflow = source('.github/workflows/annual-only-public-cutover.yml')
  const sign = workflow.indexOf('run: node scripts/build-annual-public-served-attestation.mjs'); const upload = workflow.indexOf('uses: actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a'); const finalize = workflow.indexOf('Verify the finalized Stage B artifact record binds this exact run and head'); assert.ok(sign >= 0 && sign < upload && upload < finalize)
  assert.match(workflow, /uses: actions\/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a # v7\.0\.1/); assert.match(workflow, /path: \.annual-only\/public-served\/annual-only-public-served-attestation\.json/); assert.match(workflow, /artifact_id: \$\{\{ steps\.served-attestation-upload\.outputs\.artifact-id \}\}/); assert.match(workflow, /artifact_digest: \$\{\{ steps\.served-attestation-upload\.outputs\.artifact-digest \}\}/)
  assert.doesNotMatch(workflow, /publish-annual-public-served|ACTIONS_RUNTIME_TOKEN|ACTIONS_RESULTS_URL/); assert.match(workflow, /actions: read/); assert.match(workflow, /gh api "repos\/\$GITHUB_REPOSITORY\/actions\/artifacts\/\$ARTIFACT_ID"/); assert.match(workflow, /\.workflow_run\.head_sha/); assert.match(workflow, /\.expired/); assert.match(workflow, /\.digest/)
})
test('the correction branch is excluded only from the Docs preview push trigger', () => {
  const text = source('.github/workflows/preview-docs.yml'); const branches = text.match(/branches-ignore:\n([\s\S]*?)\n\s*paths:/)?.[1] ?? ''
  assert.match(branches, /- main/); assert.match(branches, /- fix\/annual-finalized-artifact-20260905/); assert.equal((branches.match(/^\s+- /gm) ?? []).length, 2); assert.match(text, /push:/); assert.match(text, /paths:/)
})
