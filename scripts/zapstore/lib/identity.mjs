...
// protected workflow checkout, never from the release tag's tree, and it sees
// the run's real GITHUB_REF: the helper refuses any ref but protected main, and
// this lane never substitutes one.
...
// `GITHUB_WORKFLOW_REF` is `{owner}/{repo}/{path}@{ref}` and names the loaded
// definition file.
...
export function helperEnvironment(env = process.env) {
  if (env.GITHUB_REF !== undefined && env.GITHUB_REF !== PROTECTED_REF) {
    throw new Error(`refusing identity check: this run's GITHUB_REF is ${env.GITHUB_REF}, not ${PROTECTED_REF}`)
  }
  return {
    ...
    ...(env.GITHUB_REF !== undefined ? { GITHUB_REF: env.GITHUB_REF } : {}),
  }
}
...
export function verifySourceIdentity({
  ...
  const result = spawn('bash', args, { encoding: 'utf8', env: helperEnvironment(env) })
  if (result.status !== 0) {
