...
fun reduceAccountDashboardState(input: AccountDashboardInput): AccountDashboardModel {
    ...
    if (latestIsFailure(status) && status.lastFailureCategory == SyncStatusStore.FailureCategory.AUTHENTICATION)
    ...
    if (latestIsFailure(status) && status.lastFailureCategory == SyncStatusStore.FailureCategory.PERMISSION)
    ...
    if (latestIsFailure(status) && status.lastFailureCategory == SyncStatusStore.FailureCategory.CONFIGURATION)
