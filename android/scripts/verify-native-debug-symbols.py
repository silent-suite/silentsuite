...
def bundle_native_libs(bundle: pathlib.Path) -> tuple[set[tuple[str, str]], list[str]]:
    ...
    with zipfile.ZipFile(bundle) as archive:
        for name in archive.namelist():
            ...
            if parts[2] not in EXPECTED_ANDROID_ABIS:
                ...
                continue
            libs.add((parts[2], parts[3]))
    return libs, errors


def symbol_entries(symbols: pathlib.Path) -> tuple[set[tuple[str, str]], list[str]]:
...
def collect_errors(
    ...
    packaged, errors = bundle_native_libs(bundle)
    covered, symbol_errors = symbol_entries(symbols)
