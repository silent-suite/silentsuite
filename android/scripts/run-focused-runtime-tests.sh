...
if [[ "${api_level}:${shard}" == "21:mixed" ]]; then
  ...
  timeout --signal=TERM --kill-after=10s 1500s \
    ./gradlew app:connectedDebugAndroidTest --no-daemon -PrequireEtebase16Kb=true -Pandroid.testInstrumentationRunnerArguments.class="${other64_selectors}"
