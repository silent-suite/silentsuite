...
def test_fresh_emulator_runtime_shards_are_ledger_derived_and_preserve_remaining_results():
    ...
    assert (len(mixed), len(requested), len(canonical - mixed - requested), len(canonical)) == (1, 1, 92, 94)
    assert tuple(len(api36[key]) for key in api36) == (29, 19, 46)
    assert all(
    ...
    assert "expected_sizes={'21:mixed':1,'21:remaining':93,'35:all':94,'36:account-dashboard':29,'36:first-run-setup':19,'36:status-routes':46}" in assertion
    assert '"21:remaining": 92' in script
